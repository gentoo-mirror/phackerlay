...
src_configure() {
	...
	if use elpa; then
		...
		if use openmp; then
			echo "extra_compile_args += ['-felpa-openmp']" >> ${S}/siteconfig.py
			echo "extra_link_args += ['-felpa-openmp']" >> ${S}/siteconfig.py
		else
			echo "extra_compile_args += ['-felpa']" >> ${S}/siteconfig.py
			echo "extra_link_args += ['-felpa']" >> ${S}/siteconfig.py
		fi
